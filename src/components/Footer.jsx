...
export default function Footer() {
  return (
    <footer style={{ background: '#020202' }}>
      <Ticker dark />
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '72px 32px 48px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '48px' }}>
        ...
        <div style={{ gridColumn: 'span 2' }} className="col-span-2 sm:col-span-1">
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
            ...
            <div>
              ...
              <p style={{ fontFamily: 'Montserrat', color: '#4a3010', fontSize: '7px', letterSpacing: '0.45em' }}>FAISALABAD</p>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '8px', marginTop: '24px', alignItems: 'center' }}>
            <div style={{ height: '1px', width: '28px', background: 'linear-gradient(to right, transparent, #c9a84c)' }} />
            <div style={{ width: '4px', height: '4px', background: '#c9a84c', transform: 'rotate(45deg)' }} />
            <div style={{ height: '1px', width: '28px', background: 'linear-gradient(to left, transparent, #c9a84c)' }} />
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <a href="tel:03111786243" style={{ display: 'flex', alignItems: 'center', gap: '10px', color: '#444', fontFamily: 'Montserrat', fontSize: '12px', textDecoration: 'none', transition: 'color 0.3s' }}
              className="hover:text-yellow-500">
              <Phone size={12} style={{ color: '#c9a84c', flexShrink: 0 }} /> 03-111-786-243
            </a>
            <p style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', color: '#444', fontFamily: 'Montserrat', fontSize: '12px' }}>
            ...
            <a href="https://www.facebook.com/Businesshubfaisalabad/" target="_blank" rel="noreferrer"
              style={{ display: 'flex', alignItems: 'center', gap: '10px', color: '#444', fontFamily: 'Montserrat', fontSize: '12px', textDecoration: 'none', transition: 'color 0.3s' }}
              className="hover:text-yellow-500">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {['About','Features','Payment','Gallery','Location','Contact'].map(l => (
              <a key={l} href={`#${l.toLowerCase()}`}
                style={{ fontFamily: 'Montserrat', fontSize: '11px', letterSpacing: '0.12em', color: '#444', textDecoration: 'none', transition: 'color 0.3s' }}
                className="uppercase hover:text-yellow-500">{l}</a>
            ))}
          </div>
        </div>
      </div>

      <div style={{ borderTop: '1px solid rgba(201,168,76,0.06)', padding: '18px 32px', textAlign: 'center' }}>
        <p style={{ fontFamily: 'Montserrat', fontSize: '10px', color: '#222', letterSpacing: '0.08em' }}>
